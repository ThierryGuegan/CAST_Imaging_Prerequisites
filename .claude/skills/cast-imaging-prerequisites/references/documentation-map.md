...
| §2 Project scale — user/scale bands | The `poc`/`small`/`standard`/`enterprise` app-count *bands themselves* still aren't CAST-published tiers — that part stays an internal heuristic. But the `SIZING` disk/RAM *numbers* are now anchored to a real data point from the same "What hardware do I need?" PDF used for §1's Kubernetes row: managing up to 50 applications with up to 5 parallel analyses on one node needs 32 GB RAM / 2048 GB disk for that node and 64 GB RAM / 3072 GB disk for its PostgreSQL instance. This fixed a real self-contradiction — the old numbers (300 GB at `small`, 512 GB flat for dedicated PostgreSQL at every scale) were far below this CAST-documented figure for a comparable app count. `small` now uses the anchor directly; `standard`/`enterprise` are conservative multiples of it (2x/4x), explicitly labeled in the sizingHtml callout as extrapolation, not a CAST formula — CAST states app-count-to-sizing isn't linear. Don't tighten `standard`/`enterprise` further without new primary evidence; the doubling is a stated safety margin, not a researched number. | `install/requirements/hardware/` |
