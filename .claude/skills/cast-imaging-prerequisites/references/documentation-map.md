...
| §1 Platform & topology — Neo4j on its own dedicated machine ("advanced topology") | User-supplied doc excerpt confirmed this is documented for **Docker and Podman only** — not Windows, not Kubernetes. `canDedicateNeo4j(a)` in index.html gates the checkbox's visibility on this, and `state()` forces `a.neo4jDedicated` to `false` outside those two platforms so no downstream code needs its own platform check. Don't re-open this without a primary doc source showing Windows/Kubernetes support — the user-supplied excerpt outranks assuming symmetry with Docker/Podman. | `install/global/docker/` / `install/global/podman/` ("advanced topology" step) |
